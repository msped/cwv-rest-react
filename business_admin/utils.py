...
def render_to_pdf(data):
    template = render_to_string("invoice_template.html", {"invoice": data}).encode("utf-8")
    result = BytesIO()
    pdf = pisaDocument(BytesIO(template), result, encoding="utf-8")
    if not pdf.err:
...
def invoice_handler(request):
    ...
    email = EmailMessage(
        ...
        message, "noreply@cheshirewestvehicles.co.uk",
        [data["customer"]["email"]]
    )
